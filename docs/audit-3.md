...
| `pthread_mutex_t` | `std::atomic` / lock-free | Eliminates frame-drop stutter |
...
**2026 Migration:**
```cpp
// Replace mutex-guarded buffer with atomic swap
std::atomic<std::shared_ptr<FrameBuffer>> current_frame;

// Capture thread (producer)
auto new_frame = std::make_shared<FrameBuffer>(/* ... */);
current_frame.store(new_frame, std::memory_order_release);

// Render thread (consumer) - never blocks capture
auto frame = current_frame.load(std::memory_order_acquire);
if (frame) render(*frame);
```

**Risk Level:** MEDIUM (Lock convoy, potential ANR)
**Migration Effort:** MEDIUM (requires shared_ptr refactor)
```
...
| **Medium** | Triple-buffer for frames | 2 days | Eliminates mutex contention |
...
```cpp
// Before: Mutex-guarded buffer
pthread_mutex_t frame_mutex;
uint8_t* current_frame;

void producer() {
    pthread_mutex_lock(&frame_mutex);
    memcpy(current_frame, new_data, size);
...
// After: Atomic shared_ptr swap
std::atomic<std::shared_ptr<FrameBuffer>> current_frame;

void producer() {
    auto new_frame = std::make_shared<FrameBuffer>(new_data, size);
    current_frame.store(new_frame, std::memory_order_release);
}
